...
covid_cases['Digits']=covid_cases['Confirmed'].apply(lambda x: str(x)[0])
covid_cases.drop('Confirmed',axis=1,inplace=True)
covid_cases = covid_cases['Digits'].value_counts().rename('count').rename_axis('Digits').reset_index()
covid_cases = covid_cases.sort_values(by='Digits')
covid_cases['percentage'] = covid_cases['count'].apply(lambda x: str(round((x/sum(covid_cases['count']) * 100),2))+"%")
covid_cases['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
covid_cases['benford count'] = covid_cases['benford percentage'].apply(lambda x : (float(x) * sum(covid_cases['count']))/100)

# Eathquake depths
earthquake_depths = {'Digits':[1,2,3,4,5,6,7,8,9],'count':[78675,42066,34848,21630,17374,18469,13117,11400,10852]}
earthquake_depths = pd.DataFrame(earthquake_depths)
earthquake_depths['percentage'] = earthquake_depths['count'].apply(lambda x: str(round((x/sum(earthquake_depths['count']) * 100),2))+"%")
earthquake_depths = earthquake_depths.sort_values(by='Digits')
earthquake_depths['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
earthquake_depths['benford count'] = earthquake_depths['benford percentage'].apply(lambda x : (float(x) * sum(earthquake_depths['count']))/100)
...
# highways length
highways = pd.read_html('https://en.wikipedia.org/wiki/List_of_National_Highways_in_India_by_highway_number')[0]
highways = highways['Length']['km'].to_frame()
highways.columns = ['Digits']
highways.dropna(inplace=True)
highways_table = pd.DataFrame({'Total Records':[len(highways)],'Maximum Value':[highways['Digits'].max()],'Minimum Value':[highways['Digits'].min()],'Order of Magnitude':[len(str(highways['Digits'].max()))-len(str(highways['Digits'].min()))]})
highways = highways['Digits'].apply(lambda x: str(x)[0]).value_counts().rename('count').rename_axis('Digits').reset_index()
highways['percentage'] = highways['count'].apply(lambda x: str(round((x/sum(highways['count']) * 100),2))+"%")
highways = highways.sort_values(by='Digits')
highways['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
highways['benford count'] = highways['benford percentage'].apply(lambda x : (float(x) * sum(highways['count']))/100)

# area of countries
country_area = pd.read_html('https://en.wikipedia.org/wiki/List_of_countries_and_dependencies_by_area')[0].drop(
    ['Rank', 'Country / Dependency', 'Land in km2 (mi2)', 'Water in km2 (mi2)', '% water', 'Notes'], axis=1)
country_area.columns = ['Area']
country_area.drop(country_area.head(1).index, inplace=True)
country_area.drop(country_area.tail(4).index, inplace=True)
country_area['Area'] = country_area['Area'].apply(
    lambda x: int(x.split('(')[0].replace(' ', '').replace(',', '').split('.')[0]))
country_area_table = pd.DataFrame({'Total Records': [str(len(country_area))], 'Maximum Value': [country_area['Area'].max()],
                         'Minimum Value': [country_area['Area'].min()],'Order of Magnitude':[len(str(country_area['Area'].max()))-len(str(country_area['Area'].min()))]})

country_area['Area'] = country_area['Area'].apply(lambda x: str(x)[0])
country_area = country_area[country_area['Area'] != "("]
country_area = country_area['Area'].value_counts().head(9).rename('count').rename_axis('Digits').reset_index()
country_area['percentage'] = country_area['count'].apply(
    lambda x: str(round((x / sum(country_area['count']) * 100), 2)) + "%")
country_area = country_area.sort_values(by='Digits')
country_area['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
country_area['benford count'] = country_area['benford percentage'].apply(
    lambda x: (float(x) * sum(country_area['count'])) / 100)
...
stars_data = pd.read_fwf(io.StringIO(txt), infer_nrows=300)
# just rows that have a number in first columns
stars_distance = stars_data[stars_data.iloc[:,0].str.match("^[0-9][0-9]*\.$").fillna(False)]
stars_distance = stars_distance.iloc[:,-1:]
stars_distance.columns = ['Digits']
stars_distance['Digits'] = stars_distance['Digits'].apply(lambda x: int(x))
stars_distance_table = pd.DataFrame({'Total Records': [str(len(stars_distance))], 'Maximum Value': [stars_distance['Digits'].max()],
                         'Minimum Value': [stars_distance['Digits'].min()],'Order of Magnitude':[len(str(stars_distance['Digits'].max()))-len(str(stars_distance['Digits'].min()))]})
stars_distance['Digits'] = stars_distance['Digits'].apply(lambda x: str(x)[0])
stars_distance = stars_distance['Digits'].value_counts().rename('count').rename_axis('Digits').reset_index()
stars_distance['percentage'] = stars_distance['count'].apply(lambda x: str(round((x/sum(stars_distance['count']) * 100),2))+"%")
stars_distance = stars_distance.sort_values(by='Digits')
stars_distance['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
porn_views = porn_views['views'].value_counts().rename('count').rename_axis('Digits').reset_index()
porn_views['percentage'] = porn_views['count'].apply(lambda x: str(round((x/sum(porn_views['count']) * 100),2))+"%")
porn_views = porn_views.sort_values(by='Digits')
porn_views['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
porn_views['benford count'] = porn_views['benford percentage'].apply(lambda x : (float(x) * sum(porn_views['count']))/100)

# population of countries
countries_population = pd.read_html('https://en.wikipedia.org/wiki/List_of_countries_by_past_and_projected_future_population')[1].drop(['Country (or dependent territory)', '%', '%.1',
       '%.2', '%.3', '%.4', '%.5', '%.6'],axis=1)
countries_population = countries_population.stack().reset_index().drop(['level_0', 'level_1'],axis=1)
countries_population.columns = ['Digits']
countries_population_table = pd.DataFrame({'Total Records': [str(len(countries_population))], 'Maximum Value': [countries_population['Digits'].max()],
                         'Minimum Value': [countries_population['Digits'].min()],'Order of Magnitude':[len(str(countries_population['Digits'].max()))-len(str(countries_population['Digits'].min()))]})
countries_population['Digits'] = countries_population['Digits'].apply(lambda x: str(x)[0])
countries_population = countries_population['Digits'].value_counts().rename('count').rename_axis('Digits').reset_index()
countries_population['percentage'] = countries_population['count'].apply(lambda x: str(round((x/sum(countries_population['count']) * 100),2))+"%")
countries_population = countries_population.sort_values(by='Digits')
countries_population['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
exoplanet_mass.drop(exoplanet_mass.tail(7).index,inplace=True)
exoplanet_mass['Digits'] = exoplanet_mass['Digits'].apply(lambda x: str(x)[0])
exoplanet_mass = exoplanet_mass['Digits'].value_counts().rename('count').rename_axis('Digits').reset_index()
exoplanet_mass['percentage'] = exoplanet_mass['count'].apply(lambda x: str(round((x/sum(exoplanet_mass['count']) * 100),2))+"%")
exoplanet_mass = exoplanet_mass.sort_values(by='Digits')
exoplanet_mass['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
cricket_runs['Runs Digits']=cricket_runs['Runs'].apply(lambda x: str(x)[0])
cricket_runs = cricket_runs['Runs Digits'].value_counts().rename('count').rename_axis('Digits').reset_index()
cricket_runs['percentage'] = cricket_runs['count'].apply(lambda x: str(round((x/sum(cricket_runs['count']) * 100),2))+"%")
cricket_runs['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
terrorism_deaths = terrorism_deaths['deaths'].value_counts().rename('count').rename_axis('Digits').reset_index()
terrorism_deaths['percentage'] = terrorism_deaths['count'].apply(lambda x: str(round((x/sum(terrorism_deaths['count']) * 100),2))+"%")
terrorism_deaths['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
movie_revenue = movie_revenue.sort_values(by='Digits')
movie_revenue['percentage'] = movie_revenue['count'].apply(lambda x: str(round((x/sum(movie_revenue['count']) * 100),2))+"%")
movie_revenue['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
moon_craters = moon_craters['Digits'].value_counts().head(9).rename('count').rename_axis('Digits').reset_index()
moon_craters['percentage'] = moon_craters['count'].apply(lambda x: str(round((x/sum(moon_craters['count']) * 100),2))+"%")
moon_craters = moon_craters.sort_values(by='Digits')
moon_craters['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
moon_craters['benford count'] = moon_craters['benford percentage'].apply(lambda x : (float(x) * sum(moon_craters['count']))/100)

# fibonacci series
fibonacci_list=[0,1]
for i in range(2, 1000):
  nextElement = fibonacci_list[i-1] + fibonacci_list[i-2]
  fibonacci_list.append(nextElement)
fibonacci_list_table = pd.DataFrame({'Total Records':[len(fibonacci_list)],'Maximum Value':'2.68638100244849997721985346288 × 10²⁹','Minimum Value':[min(fibonacci_list)],'Order of Magnitude':[len(str(max(fibonacci_list)))-min(fibonacci_list)]})
fibonacci_list = pd.DataFrame(fibonacci_list,columns=['Digits'])
fibonacci_list['Digits'] = fibonacci_list['Digits'].apply(lambda x: str(x)[0])
fibonacci_list = fibonacci_list['Digits'].value_counts().head(9).rename('count').rename_axis('Digits').reset_index()
fibonacci_list['percentage'] = fibonacci_list['count'].apply(lambda x: str(round((x/sum(fibonacci_list['count']) * 100),2))+"%")
fibonacci_list['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
disasters_deaths['percentage'] = disasters_deaths['count'].apply(lambda x: str(round((x/sum(disasters_deaths['count']) * 100),2))+"%")
disasters_deaths['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
disasters_deaths['benford count'] = disasters_deaths['benford percentage'].apply(lambda x : (float(x) * sum(disasters_deaths['count']))/100)

# infectious disease
global_diseases = {'Digits':[1,2,3,4,5,6,7,8,9],'count':[332,164,130,105,72,53,45,50,33]}
global_diseases = pd.DataFrame(global_diseases)
global_diseases['percentage'] = global_diseases['count'].apply(lambda x: str(round((x/sum(global_diseases['count']) * 100),2))+"%")
global_diseases = global_diseases.sort_values(by='Digits')
global_diseases['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
global_diseases['benford count'] = global_diseases['benford percentage'].apply(lambda x : (float(x) * sum(global_diseases['count']))/100)
...
sunni_muslims['percentage'] = sunni_muslims['count'].apply(lambda x: str(round((x/sum(sunni_muslims['count']) * 100),2))+"%")
sunni_muslims = sunni_muslims.sort_values(by='Digits')
sunni_muslims['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
...
disasters_damage['percentage'] = disasters_damage['count'].apply(lambda x: str(round((x/sum(disasters_damage['count']) * 100),2))+"%")
disasters_damage['benford percentage'] = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6]
disasters_damage['benford count'] = disasters_damage['benford percentage'].apply(lambda x : (float(x) * sum(disasters_damage['count']))/100)

def onload_benfordfig():
    fig = go.Figure()
    fig.add_trace(
        go.Bar(x=covid_cases['Digits'], y=covid_cases['count'], text=covid_cases['percentage'], showlegend=False,
               name='Leading Digit', marker_color='yellow'))
...
benford_datasets = {'Count of Covid-19 cases worldwide':1,'Depth of Earthquakes globally in km':2,'Length of Highways in India':3,'Total Area of Countries in km²':4,'Distance of Stars from Earth in light years':5,'Views count on Pornhub':6,'Population of Countries':7,'Mass of Exoplanets in Earth mass':8,'Runs scored by Batsmen in Test Cricket':9,'People killed by Terrorism':10,'Revenue generated by Movies in $US':11,"Diameter of Moon's craters in km":12,'First 1000 Fibonnaci numbers':13,'Deaths due to Natural Disasters':14,'Count of  Global Infectious Disease cases':15,'Number of Sunni Muslims per country':16,'Damage suffered due to Natural Disasters globally in $US':17}

# -----------------------------------------------ZIPF'S LAW-----------------------------------------------------------------------------------
# brown corpus
word_count = len(brown.words())
browncorpus = pd.DataFrame(brown.words(), columns=['word'])
browncorpus['word'] = browncorpus['word'].apply(
    lambda x: None if x in [",", ".", "``", "''", ";", "?", "--", "(", ")", ":", "!", '''"''', "'"] else x)
browncorpus.dropna(inplace=True)
browncorpus['word'] = browncorpus['word'].apply(lambda x: x.lower() if x != "I" else x)
browncorpus = browncorpus['word'].value_counts().head(100).rename('count').rename_axis('word').reset_index()
browncorpus['percentage'] = browncorpus['count'].apply(lambda x: str(round((x / word_count) * 100, 2)) + "%")
zipf_dist = []
for i in range(1, len(browncorpus) + 1):
    zipf_dist.append(int(browncorpus['count'][0] / i))
browncorpus['zipf_dist'] = zipf_dist
browncorpus.index = range(1, len(browncorpus) + 1)

# marathi corpus
marathi_word_count = len(nltk.corpus.indian.words('marathi.pos'))
marathi_corpus = pd.DataFrame(nltk.corpus.indian.words('marathi.pos'),columns=['word'])
marathi_corpus['word'] = marathi_corpus['word'].apply(lambda x: None if x in [",",".","``","''",";","?","--","(",")",":","!",'''"''',"'"] else x)
marathi_corpus = marathi_corpus['word'].value_counts().head(50).rename('count').rename_axis('word').reset_index()
marathi_corpus['percentage'] = marathi_corpus['count'].apply(lambda x: str(round((x/marathi_word_count)*100,2))+"%")
zipf_dist=[]
for i in range(1,len(marathi_corpus)+1):
  zipf_dist.append(int(marathi_corpus['count'][0]/i))
marathi_corpus['zipf_dist'] = zipf_dist
marathi_corpus.index = range(1,len(marathi_corpus)+1)

# bible corpus
bible_word_count = len(nltk.corpus.gutenberg.words('bible-kjv.txt'))
bible_corpus = pd.DataFrame(nltk.corpus.gutenberg.words('bible-kjv.txt'),columns=['word'])
bible_corpus['word'] = bible_corpus['word'].apply(lambda x: None if x in [",",".","``","''",";","?","--","(",")",":","!",'''"''',"'",'1','2','3','4','5','6','7','8','9','0'] else x)
bible_corpus.dropna(inplace=True)
bible_corpus = bible_corpus['word'].value_counts().head(100).rename('count').rename_axis('word').reset_index()
bible_corpus['percentage'] = bible_corpus['count'].apply(lambda x: str(round((x/bible_word_count)*100,2))+"%")
zipf_dist=[]
for i in range(1,len(bible_corpus)+1):
  zipf_dist.append(int(bible_corpus['count'][0]/i))
bible_corpus['zipf_dist'] = zipf_dist
bible_corpus.index = range(1,len(bible_corpus)+1)

# surnames
surnames = pd.read_html('https://en.wikipedia.org/wiki/List_of_most_common_surnames_in_North_America')[-2].drop(['Rank', '1990 rank', 'Change', 'Frequency per 100,000', 'Cumulative per 100,000', 'White', 'Black',
       'Asian orPacific Islander', 'Native Americanor Alaskan', 'Multiracial',
       'Hispanic'],axis=1)
surnames.columns = ['surname','count']
zipf_dist=[]
for i in range(1,len(surnames)+1):
  zipf_dist.append(int(surnames['count'][0]/i))
surnames['zipf_dist'] = zipf_dist
surnames.index = range(1,len(surnames)+1)

# cities area
cities = pd.read_html('https://en.wikipedia.org/wiki/List_of_largest_cities')[1]
city_area = cities['City[a]']
city_area['area'] = cities['City proper[c]']['Area(km2)']
city_area.columns = ['city','area']
city_area.dropna(inplace=True)
city_area['area'] = city_area['area'].apply(lambda x: str(x).replace(',',''))
city_area['area'] = city_area['area'].apply(lambda x: str(x).split('.')[0])
city_area['area'] = city_area['area'].apply(lambda x: int(str(x).split('[')[0]))
city_area = city_area.sort_values(by='area',ascending=False).reset_index(drop=True)
city_area = city_area.head(50)
zipf_dist=[]
for i in range(1,len(city_area)+1):
  zipf_dist.append(int(city_area['area'][0]/i))
city_area['zipf_dist'] = zipf_dist
...
dog_attack = dog_attack.sort_values(by='age')
zipf_dist=[]
for i in range(1,len(dog_attack)+1):
  zipf_dist.append(int(dog_attack['count'][0]/i))
dog_attack['zipf_dist'] = zipf_dist
dog_attack.index = range(1,len(dog_attack)+1)
...
# ---------------------------------------------BREVITY LAW--------------------------------------------------------------------------------------------
# browncorpus2
browncorpus2 =  pd.DataFrame(brown.words(),columns=['word'])
browncorpus2['word'] = browncorpus2['word'].apply(lambda x: None if x in [",",".","``","''",";","?","--","(",")",":","!",'''"''',"'"] else x)
browncorpus2.dropna(inplace=True)
browncorpus2['word'] = browncorpus2['word'].apply(lambda x: x.lower() if x != "I" else x)
browncorpus2 = browncorpus2['word'].value_counts().head(250).rename('count').rename_axis('word').reset_index()
browncorpus2.index = range(1,len(browncorpus2)+1)
browncorpus2['word size'] = browncorpus2['word'].apply(lambda x: 'words having 3 or less letters' if len(x)<4 else 'words having 4 or more letters')
...
# ------------------------------------------Titius–Bode law---------------------------------------------------------------------------------------
# planet distance
planets_distance = pd.read_html('https://en.wikipedia.org/wiki/Titius%E2%80%93Bode_law')[0].drop(['m','T–B rule distance (AU)','Deviation from prediction1'],axis=1)
planets_distance.drop(planets_distance.tail(5).index,inplace=True)
planets_distance['Planet'] = planets_distance['Planet'].apply(lambda x: '('+x.split('2')[0]+')' if x[-1]=='2' else x)
planets_distance['T–B rule distance (AU)']=[0.4,0.7,1,1.6,2.8,5.2,10.0,19.6,38.8,77.2]
...
# microprocessor
transistor_mp = pd.read_html('https://en.wikipedia.org/wiki/transistor_count')[1].drop(['Area (mm2)'],axis=1)
...
transistor_mp['transistor count'] = transistor_mp['transistor count'].apply(lambda x: x.split(" (")[0])
transistor_mp['transistor count'] = transistor_mp['transistor count'].apply(lambda x: x.split(" ")[0])
transistor_mp['transistor count'] = transistor_mp['transistor count'].apply(lambda x: int(x.replace(",","")))
transistor_mp['Processor'] = transistor_mp['Processor'].apply(lambda x: x.split("(")[0])
transistor_mp['Date'] = pd.to_datetime(transistor_mp['Date'])

# GPU
transistor_gpu = pd.read_html('https://en.wikipedia.org/wiki/Transistor_count')[2].drop(['Manufacturer(s)','Area','Ref'],axis=1)
transistor_gpu.columns=['Processor','transistor count','Date','Designer','MOS process']
transistor_gpu.drop(transistor_gpu[transistor_gpu['transistor count']=='Unknown'].index,inplace=True)
transistor_gpu['Date'] = transistor_gpu['Date'].apply(lambda x: x.split(" ")[1] if " " in x else x)
transistor_gpu['transistor count'].astype(str).astype(np.uint64)
transistor_gpu['Date'] = pd.to_datetime(transistor_gpu['Date'])
...
transistor_ram['Date'] = pd.to_datetime(transistor_ram['Date'])

# Flash memory
transistor_flash = pd.read_html('https://en.wikipedia.org/wiki/Transistor_count')[5].drop(['Chip name','MOS process','Area','Ref'],axis=1)
transistor_flash.columns = ['Capacity (bits)','Flash type','transistor count','Date','Designer']
transistor_flash['Date'] = transistor_flash['Date'].apply(lambda x: x.split(" ")[1] if " " in x else x)
transistor_flash['transistor count'].astype(str).astype(np.uint64)
transistor_flash['Date'] = pd.to_datetime(transistor_flash['Date'])
...
def update_graph(graph_chosen):
    ...
    if graph_chosen==2:
        ...
        fig.add_trace(
            go.Bar(x=earthquake_depths['Digits'], y=earthquake_depths['count'], text=earthquake_depths['percentage'],
                   showlegend=False, name='Leading Digit', marker_color='yellow'))
        fig.update_layout(xaxis=dict(
            type='category',
            categoryarray=['1', '2', '3', '4', '5', '6', '7', '8', '9'],
    ...
    if graph_chosen==7:
        ...
        fig.add_trace(go.Bar(x=countries_population['Digits'], y=countries_population['count'],
                             text=countries_population['percentage'], showlegend=False, name='Leading Digit',
                             marker_color='yellow'))
        fig.update_layout(xaxis=dict(
            type='category',
            categoryarray=['1', '2', '3', '4', '5', '6', '7', '8', '9'],
